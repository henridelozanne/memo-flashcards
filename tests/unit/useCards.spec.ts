...
describe('useCards', () => {
  ...
  describe('updateCard', () => {
    it('should update an existing card', async () => {
      ...
      expect(mockSqliteConnection.get).toHaveBeenCalledWith('SELECT * FROM cards WHERE id = ? AND deleted_at IS NULL', [
        'test-id',
      ])
    ...
    it('should throw error if card not found', async () => {
      ...
      await expect(composable.updateCard('nonexistent-id', 'Q', 'A')).rejects.toThrow('Carte introuvable')
    })
  })
  ...
  describe('deleteCard', () => {
    ...
    it('should throw error if card not found', async () => {
      ...
      await expect(composable.deleteCard('nonexistent-id')).rejects.toThrow('Carte introuvable')
    })
  })
  ...
  describe('getCard', () => {
    ...
    it('should return null if card is deleted', async () => {
      const mockCards = [{ id: 'card-1', question: 'Q1', deleted_at: Date.now() }]
