    ...
    localCollections.forEach((local) => {
      ...
      if (!remote || localUpdatedAt > remoteUpdatedAt) {
        toUpsert.push({
          ...
          rejected_ai_cards: local.rejected_ai_cards ?? null,
    ...
    for (const remote of remoteCollections) {
      ...
      if (!local || remoteUpdatedAt > localUpdatedAt) {
        ...
        if (existingCount && existingCount.count > 0) {
          ...
          await db.run(
            `UPDATE collections 
             SET user_id = ?, name = ?, color = ?, card_background = ?, rejected_ai_cards = ?, created_at = ?, updated_at = ?, deleted_at = ?
             WHERE id = ?`,
            [
              remote.user_id,
              remote.name,
          ...
          // eslint-disable-next-line no-await-in-loop
          await db.run(
            `INSERT INTO collections (id, user_id, name, color, card_background, rejected_ai_cards, created_at, updated_at, deleted_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
